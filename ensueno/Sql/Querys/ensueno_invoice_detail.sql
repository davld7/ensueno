...
begin
    declare @S as int
    select @S=[product_stock]
    from PRODUCTS
    where product_active=1 and product_id=@product_id
    if(@S>@units)
BEGIN
        declare @Price as MONEY
        select @Price =[product_unit_price]
        from PRODUCTS
        where product_active=1 and product_id=@product_id
        insert into INVOICE_DETAIL
            (invoice_id,product_id,units,price)
        Values(@invoice_id, @product_id, @units, @Price)
        declare @Stock as int
        select @Stock=[product_stock]
        from PRODUCTS
        where product_active=1 and product_id=@product_id
...
begin
    ...
    select @Stock_P =[product_stock]
    from PRODUCTS
    where product_active=1 and product_id=@product_id
    declare @Units_I_D as int
    select @Units_I_D=[units]
    from INVOICE_DETAIL
    where invoice_detail_active=1 and invoice_id=@invoice_id and product_id=@product_id
    declare @New_Stock as int
...
begin
    ...
    select @Stock_Prod=[product_stock]
    from PRODUCTS
    where product_active=1 and product_id=@Product_id
    declare @New_Stock as int
...
begin
    ...
    select @Restore_Stock=[units]
    from INVOICE_DETAIL
    where invoice_detail_active=0 and invoice_id=@Product_id and product_id=@Product_id
    ...
    select @Stock_Product=[product_stock]
    from PRODUCTS
    where product_active=1 and product_id=@Product_id
    declare @New_Stock_Product AS INT
...
go
--Procedimiento para leer registros con id
...
as
begin
    SELECT cast(round(SUM(total),2)as decimal (20,2)) as 'Total'
    from INVOICE_DETAIL
...
SELECT max(invoice_id) as 'Valor ID'
FROM INVOICE
select *
from PRODUCTS